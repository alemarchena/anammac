import {datos,LlenarFormulario, Info} from './formulario.js?a=70'
import {CerrarModal,MostarImagenLogin} from './modal.js?a=70'
import {ShowMessage} from './showmessage.js?a=70'
import {LoggMenu} from './logincheck.js?a=70'
import {DesActivarSistema, ActivarSistema,LoadURL} from './cargarsistema.js?a=70'
import {ListaEstados} from './estados.js?a=70'
import {ListaSangres} from './sangres.js?a=70'
import {ListaEspecialidades} from './especialidades.js?a=70'
import {ListaTallas} from './tallas.js?a=70'
import {ConvierteaDMA} from './clases.js?a=70'
...
export function Ingresa(tipo,id,email,usuario)
{
    ...
    fetch('./controladores/consultaafiliado.php?a=70',{method:'POST',body:JSON.stringify(datosconsulta),headers:{'Content-Type':'application/Json'}})
    .then(response=>
    {
...
function GuardarDatos(email)    
{
    ...
    fetch("./controladores/guardarafiliacion.php?a=70",{method:'POST',body: JSON.stringify( datosconsulta ),headers:{'Content-Type':'application/json'}})   
    .then(response =>{
        return response.json();
    }).then(data => {
        if(data == 1)
